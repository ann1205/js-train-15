...
function roundObjectValues(obj) {
  ...
  const roundedEntries = Object.fromEntries(filteredEntries);
...
function calculateVolumeCylinder(radius, height) {
  ...
  const volumeCelinder = Math.ceil(Math.PI * Math.pow(radius, 2) * height);
  return volumeCelinder;
}
...
function getFractionalPart(num) {
  ...
  const roundNumb = Math.trunc(num);
  const diffFractionalPart = Math.fround(num - roundNumb);
  return diffFractionalPart;
}
...
function compareAndRound(num1, num2) {
  ...
  const biggestNumb = Math.round(Math.max(num1, num2));
  return biggestNumb;
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sumInvestment = Math.round(
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPriceRounded = Math.fround(totalPrice);
  const maxPriceRounded = Math.fround(maxPrice);
  if (totalPriceRounded >= maxPriceRounded) {
    return true;
  } else {
    return false;
  }
